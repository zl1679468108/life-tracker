...
| T75 | P0 | v1.4.1 高保真交互地图 1:1 实现 | frontend/backend | done | 2026-06-30 以 Web / PWA 作为当前验收口径完成闭环：第三轮真实登录态截图回归已完成，视觉整改项已由 T76 关闭；Android 冒烟不再作为当前版本阻塞项 |
| T76 | P0 | v1.4.1 视觉差异审计与页面还原整改 | frontend/docs | done | 2026-06-30 已完成第三轮真实登录态 viewport 截图回归，补齐消息好友弹层、聊天详情、首页、工作台、我的、账号、主题、语言、物品/待办表单、分类/位置/模板/借用管理页验证；截图见 `docs/qa/ui-audit-20260630/` |
| T77 | P0 | v1.4.2 视觉统一减法收尾 | frontend/docs | done | 2026-07-01 已完成批注整改、消息页重构、逐页审美回归与 mock UI 自动化验收；见 `../docs/qa/ui-integration-2026-06-30T19-15-08-628Z/` |
| T78 | P0 | Web 真实表单 CRUD 冒烟与问题收敛 | frontend/qa | done | 2026-07-02 已完成物品、待办、分类、位置、模板、借用真实 Web 冒烟；修复位置重复回显和借用重复创建兜底，记录见 `docs/qa/T78_REAL_CRUD_20260702.md` |
...
| T74.3 | P0 | 第三阶段：按定稿 1:1 实现前端页面 | frontend | done | 2026-06-28 T75 页面与交互实现已完成，外部验收已按当前 Web / PWA 口径闭环 |
...
| T76.3 | P0 | 首页按交互地图重做层级与信息密度 | frontend/home | done | 2026-06-30 已结合真实 QA 数据继续压紧首页首屏，第三轮 viewport 截图确认今日总览、问候语、统计卡、快捷操作和最近待办在单屏内层级稳定；见 `docs/qa/ui-audit-20260630/home-viewport.png` |
| T76.4 | P0 | 工作台入口卡片与管理分组按交互地图收敛 | frontend/workbench | done | 2026-06-30 已继续收紧工作台 hero、核心入口卡高度和管理分组间距，第三轮 viewport 截图确认首屏密度与信息节奏稳定；见 `docs/qa/ui-audit-20260630/workbench-viewport.png` |
| T76.5 | P0 | 消息列表/好友/聊天三页按第 5 排重新对齐 | frontend/messages | done | 2026-06-30 已补充消息页自动化可测钩子并完成第三轮真实登录态截图回归，拿到列表态、好友列表弹层和聊天详情稳定截图；见 `docs/qa/ui-audit-20260630/messages-list-viewport.png`、`messages-sheet-friends-viewport-6s.png`、`messages-chat-detail-viewport.png` |
| T76.6 | P0 | 我的外层页与账号/主题页按第 6 排统一 | frontend/profile/settings | done | 2026-06-30 已完成“我的”外层、账号管理、主题、语言四页第三轮 viewport 验收，并修复版本信息改为读取 `app.json` 后显示 `v1.4.1`；见 `docs/qa/ui-audit-20260630/settings-viewport.png`、`settings-account-viewport.png`、`settings-theme-viewport.png`、`settings-language-viewport.png` |
...
| T76.8 | P1 | 物品与待办新增/编辑页按通用表单稿复核 | frontend/items/todos | done | 2026-06-30 已完成物品/待办新增页第三轮 viewport 验收，确认字段分组、单行选择入口、固定底部操作条和首屏节奏稳定；见 `docs/qa/ui-audit-20260630/item-create-viewport.png`、`todo-create-viewport.png` |
| T76.9 | P1 | 分类/位置/模板/借用管理页去旧版样式并统一到 v1.4.1 | frontend/settings | done | 2026-06-30 已完成分类、位置、模板、借用管理页第三轮 viewport 验收，并继续去除借用页残留说明性头部、压紧统计与筛选密度；见 `docs/qa/ui-audit-20260630/category-manage-viewport.png`、`location-manage-viewport.png`、`templates-viewport.png`、`borrowings-viewport.png` |
...
- 继续清理说明型文案、重复入口、悬浮装饰和双层容器，统一一级页标题栏、列表头部和固定底部操作区。
- 以“移动产品感优先、管理后台感降低”为原则，按页面逐项收口，不改后端接口和数据库。

### 任务拆解

| ID | 优先级 | 任务 | 模块 | 状态 | 备注 |
|---|---|---|---|---|---|
| T77.1 | P0 | 一级页标题栏统一到单主标题 + 统一动作区 | frontend/home/workbench/messages/profile | done | 2026-07-01 已统一首页/工作台/消息/我的主标题与动作区节奏，首页未读红点归位 |
| T77.2 | P0 | 首页/工作台继续去说明型信息与冗余入口 | frontend/home/workbench | done | 2026-07-01 已移除首页统计角标、工作台多余入口卡与说明型头部内容 |
| T77.3 | P0 | 列表页头部去说明文案并统一工具区权重 | frontend/items/todos | done | 2026-07-01 已清理物品/待办列表眉标题，待办头部改为单主标题 + 轻量工具区 |
| T77.4 | P0 | 表单页底部固定操作区去双层容器 | frontend/items/todos/account | done | 2026-07-01 已统一物品/待办/账号页底部操作区，去掉额外壳层卡片 |
| T77.5 | P1 | 设置二级页统一为轻量单选/操作列表模板 | frontend/settings | done | 2026-07-01 已收紧账号、主题、语言二级页描述与边框层级 |
| T77.6 | P1 | 消息页按 IM 主视觉继续减法统一 | frontend/messages | done | 2026-07-01 已压低头像带与系统消息块权重，让会话列表回到主视觉中心 |
| T77.7 | P1 | 逐页审美回归截图并同步任务状态 | docs/qa | done | 2026-07-01 已基于 mock UI 自动化报告 `../docs/qa/ui-integration-2026-06-30T19-15-08-628Z/` 完成回归，27 个页面与 10 个表单全部 PASS，含消息发送表单；脚本已固定 QA 语言并补齐中英选择器兼容 |

## T78 Web 真实表单 CRUD 冒烟与问题收敛

目标：

- 不走 mock UI 脚本，直接通过应用内浏览器操作真实 Web 页面，把表单相关 CRUD 流程至少跑一遍。
- 发现问题先记录为可复现事实，再决定是否拆分修复任务。

### 任务拆解

| ID | 优先级 | 任务 | 模块 | 状态 | 备注 |
|---|---|---|---|---|---|
| T78.1 | P0 | 物品新增真实提交流程验证 | frontend/items | done | 2026-07-01 已修复 Web `DatePicker` 用户手势丢失问题，并补齐 `docs/database-init.sql` 中 `life_items.barcode` 字段；真实 Web 富字段新增 `QA 全字段物品 0701-F` 已成功提交并返回列表，验证通过图片上传、条码、到期日期与价值信息链路。当前仅保留一个兼容性风险：价值区在自动化滚动时字段焦点容易错位，手工操作未见阻塞 |
| T78.2 | P0 | 待办新增真实提交流程验证 | frontend/todos | done | 2026-07-01 已修复 Web `DatePicker` 在待办页的事件/挂载不稳定问题：原实现会导致日期与提醒时间未稳定写入状态，创建后编辑页不回显。修复后，真实 Web 新增 `QA 日期修复待办 0701-C` 已成功在列表展示截止日期，并在编辑页正确回显 `due_date/reminder_date`。历史样本 `QA 全字段待办 0701-B` 为修复前数据，可保留作对照 |
| T78.3 | P0 | 分类管理新增/删除真实流程验证 | frontend/categories | done | 2026-07-01 已修复分类新增后的前端重复收口；真实 Web 单次新增 `QA 修复验证分类 0701-D` 后仅新增一条记录 |
| T78.4 | P1 | 位置/模板/借用真实表单流程补测 | frontend/settings | done | 2026-07-02 应用内浏览器真实验证：位置新增先复现重复回显，修复后 `QA 位置修复 04752` 仅回显一次；模板页真实账号为空且无独立新建入口，记录为当前产品边界；借用新增出现成功 Toast，归还确认后计数更新为借出中 2 / 已归还 1 |
| T78.5 | P1 | 将真实浏览器复现结果沉淀到 QA 记录 | docs/qa | done | 2026-07-02 已新增 `docs/qa/T78_REAL_CRUD_20260702.md`，截图保存在 `docs/qa/t78-real-crud-20260702/`；验证通过 `cd frontend && npx tsc --noEmit`、`cd backend && npm run build`、`cd frontend && npm run build:web`；追加整理历史 `screens.integration.test.ts`，前端 Jest 已恢复到 11/11 suites、95/95 tests 通过；全量 `--detectOpenHandles` 已正常退出，并修复日历网格跨时区重复 key 警告，见 `docs/qa/T78_CHANGESET_NOTES_20260702.md` |
...
最后更新：2026-07-02（T78 真实 Web 表单 CRUD 冒烟完成；位置、模板、借用补测已记录并完成关键修复）
