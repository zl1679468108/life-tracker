...
export const useCategoryStore = create<CategoryState>((set, get) => ({
  ...
  addCategory: async (category) => {
    ...
    try {
      ...
      set((state) => {
        // 仅当新分类属于当前作用域时才加入列表
        const inScope = !state.loadedScope || state.loadedScope === 'all' || state.loadedScope === data.type;
        return {
          categories: state.categories.some((existing) => existing.id === data.id)
            ? state.categories.map((existing) => (existing.id === data.id ? data : existing))
            : (inScope ? [...state.categories, data] : state.categories),
          loading: false,
        };
      });
      // 同步更新缓存（缓存保存全量数据）
      const cached = await cache.get<LifeCategory[]>(CATEGORIES_CACHE_KEY);
      const allCached = Array.isArray(cached) ? cached : [];
      await cache.set(CATEGORIES_CACHE_KEY, allCached.some((c) => c.id === data.id)
        ? allCached.map((c) => (c.id === data.id ? data : c))
        : [...allCached, data]);
    } catch (error) {
  ...
  updateCategory: async (id, updates) => {
    ...
    try {
      ...
      const cached = await cache.get<LifeCategory[]>(CATEGORIES_CACHE_KEY);
      const allCached = Array.isArray(cached) ? cached : [];
      await cache.set(CATEGORIES_CACHE_KEY, allCached.map((c) => (c.id === id ? { ...c, ...updates, ...data } : c)));
    } catch (error) {
      set({ error: (error as Error).message, loading: false });
      throw error;
    }
  ...
  deleteCategory: async (id) => {
    ...
    try {
      ...
      const cached = await cache.get<LifeCategory[]>(CATEGORIES_CACHE_KEY);
      const allCached = Array.isArray(cached) ? cached : [];
      await cache.set(CATEGORIES_CACHE_KEY, allCached.filter((c) => c.id !== id));
    } catch (error) {
      set({ error: (error as Error).message, loading: false });
      throw error;
    }
